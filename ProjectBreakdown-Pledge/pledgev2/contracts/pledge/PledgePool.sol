...
contract PledgePool is ReentrancyGuard, SafeTransfer, multiSignatureClient{
    ...
    */
    ...
    /*
        Solidity 处理 浮点数（小数） 的方式。
        先乘后除,这其实是 “放大 -> 运算 -> 缩小” 的标准流程
        遇到10/30这样的,最终结果是 9.999...90，而不是 10。
        这消失的“0.000...1”去哪了？这就是所谓的 Rounding Error（舍入误差）。
        谁吃亏了？ 在这个 claimLend（领取）函数中，用户吃了一点点亏（少了 $10^{-17}$ 个代币）。
        谁占便宜了？ 协议（池子）。这极小部分的“残渣”会留在合约的余额里，成为永远取不出来的 Dust（粉尘）。
        Web3 设计原则： 宁可让用户少领一点点粉尘，也绝不能让用户多领一点点。因为如果每人都多领 $0.000...1$，最后一个人取款时，合约里的钱就会不够，导致整个交易失败（Panic）。

        3. 如何保证份额计算“不减少”？虽然上面看到确实少了一丁点，但为什么我们说它“保证了份额”？
        精度足够高： 1e18 的精度意味着误差在小数点后 18 位。对于大多数代币（如 BUSD, USDT, ETH），这个误差的价值远远低于 1 亿分之一美分。
        在业务层面，这被视为“相等”。先乘后除的保护： 如果你不使用 calDecimal 放大，直接 10 / 30 结果就是 $0$（用户丢了 $100\%$ 的钱）。用了放大，用户只丢了 $0.00000000000000001\%$ 的钱。

        极端情况：如果分母非常大怎么办？
        如果你遇到 $1 / 10^{20}$ 这种极其悬殊的比例：如果放大倍数 calDecimal 只有 $10^{18}$，那么计算结果依然会是 $0$。
        解决方案： 这就是为什么在处理一些价值极高（如 BTC）或者精度极小的业务时，有些协议会使用 calDecimal = 1e27（Ray 精度）甚至更高。
    */
    /**
     * 出借人领取 spToken 凭证
